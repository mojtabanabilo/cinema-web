...
const Kids = ({favHC}) => {
    ...
    const newKids = useSelector(state => state.selectItem);
    const dispatch = useDispatch();

    useEffect(() => {
        !newKids.length && dispatch(fetchUsers());
    }, [])

    return (
        <div className='cards' data-aos="fade-right" data-aos-delay="100">
            ...
            {
                newKids[3] ? newKids[3].slice(0,4).map(i => <div key={i.id} className='card'>
                    ...
                    <div className='votes'>
                        ...
                        </div>
                        {
                            star === false ? <img src={whiteStar} className='stars' alt='stars' onClick={() => {
                                setStar(true);
                                starHandler(newKids[3] ,i.id)
                                if(starHandler) setFavorite(prev => [...prev, starHandler(newKids[3] ,i.id)])
                            }}/> : 
